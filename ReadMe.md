...
1. In 1-2 sentences, explain what React's `useRouteMatch` hook is used for.
    ...
    useRouteMatch is a hook from react-router that lets you dynamically route to multiple locations. 
	declared like so: const { path, url } = useRouteMatch();
	you simply set the route as path, and the link as url. with both adding on the deeper individual element.

2. How would you explain form validation to someone who has never programmed before?

    you create a template of a form and decide what is required to be put in that form. 
	including what type of data you want. and then handle the errors that users would make programatically.

3. In 1-2 sentences, define end to end testing. 
    end to end testing is using ui to mimic how a user would go through your form, simulating real events and clicks. 
	Cypress is end to end.
